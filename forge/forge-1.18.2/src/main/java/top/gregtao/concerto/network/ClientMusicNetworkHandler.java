...
public class ClientMusicNetworkHandler {
    ...
    public static void accept(Player player, UUID uuid, Minecraft client) {
        if (!WAIT_CONFIRMATION.containsKey(uuid)) {
            player.displayClientMessage(new TranslatableComponent("concerto.confirm.not_found"), false);
        } else {
            MusicDataPacket packet = WAIT_CONFIRMATION.get(uuid);
            MinecraftServer server = client.getSingleplayerServer();
            if (server != null) {
                Player from = server.getPlayerList().getPlayerByName(packet.from);
                if (from != null) from.displayClientMessage(new TranslatableComponent("concerto.confirm.accept_response", player.getName().getString()), false);
            }
            MusicPlayer.INSTANCE.playTempMusic(packet.music);
            WAIT_CONFIRMATION.remove(uuid);
            player.displayClientMessage(new TranslatableComponent("concerto.confirm.accept"), false);
        }
    }
    ...
    public static void rejectAll(Player player, Minecraft client) {
        ...
        WAIT_CONFIRMATION.forEach((uuid, packet) -> {
            if (server != null) {
                Player from = server.getPlayerList().getPlayerByName(packet.from);
                if (from != null) from.displayClientMessage(new TranslatableComponent("concerto.confirm.reject_response", player.getName().getString()), false);
            }
        });
        WAIT_CONFIRMATION.clear();
        player.displayClientMessage(new TranslatableComponent("concerto.confirm.reject"), false);
    }
            ...
            if (player != null && playerName.equals(player.getName().getString())) {
                ...
                if (args.length > 3 && !Minecraft.getInstance().isLocalServer() && args[3].equals("Invite")) {
                    if (ClientConfig.INSTANCE.options.joinAgentWhenInvited) {
                        ...
                        player.displayClientMessage(new TranslatableComponent("concerto.agent.invite")
