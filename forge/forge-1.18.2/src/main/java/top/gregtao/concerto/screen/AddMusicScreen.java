...
public class AddMusicScreen extends ApplyDraggedFileScreen {
    ...
    public AddMusicScreen(Screen parent) {
        super(new TranslatableComponent("concerto.screen.manual_add"), parent);
    }
    ...
    private Consumer<String> methodSafeWrapper(Consumer<String> method) {
        return str -> {
            try {
                ...
                AddMusicScreen.this.displayAlert(new TranslatableComponent("concerto.fail"));
            }
        };
    }
    ...
    protected void init() {
        super.init();
        this.addLabel(new TranslatableComponent("concerto.screen.add.local_file"), this.width / 2, 20,
                methodSafeWrapper(str -> {
                    try {
                        MusicPlayer.INSTANCE.addMusicHere(new LocalFileMusic(str), true);
                    } catch (UnsafeMusicException e) {
                        this.displayAlert(new TranslatableComponent("concerto.error.invalid_path"));
                    }
                }));
        this.addLabel(new TranslatableComponent("concerto.screen.add.local_file.folder"), this.width / 2, 45, str -> ConcertoRunner.run(() -> {
        ...
        this.addLabel(new TranslatableComponent("concerto.screen.add.internet"), this.width / 2, 70,
                methodSafeWrapper(str -> MusicPlayer.INSTANCE.addMusicHere(new HttpFileMusic(str), true)));
        this.addLabel(new TranslatableComponent("concerto.screen.add.netease_cloud"), this.width / 2, 95,
        ...
        this.addLabel(new TranslatableComponent("concerto.screen.add.netease_cloud.playlist"), this.width / 2, 120, methodSafeWrapper(str -> {
        ...
        this.addLabel(new TranslatableComponent("concerto.screen.add.netease_cloud.album"), this.width / 2, 145, methodSafeWrapper(str -> {
            NeteaseCloudPlaylist playlist = new NeteaseCloudPlaylist(str, false);
            playlist.load(() -> Minecraft.getInstance().setScreen(new PlaylistPreviewScreen(playlist, this)));
        }));
        this.addLabel(new TranslatableComponent("concerto.screen.add.qq"), this.width / 2, 170,
        ...
        this.addLabel(
                new TranslatableComponent("concerto.screen.add.kugou"), this.width / 2, 195,
