...
public class URLImageWidget implements Renderable, LayoutElement, AutoCloseable {
    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        if (this.border) GuiComponent.renderOutline(matrices, this.x, this.y, this.width, this.height, 0xffffffff);
        Font textRenderer = Minecraft.getInstance().font;
        if (this.url == null || this.texture == null) {
            GuiComponent.drawCenteredString(
                matrices,textRenderer, Component.translatable("concerto.screen.url_image.empty"),
            ...
            if (image != null && this.state == State.READY) {
                ...
                GuiComponent.drawCenteredString(
                        matrices, textRenderer, Component.translatable("concerto.screen.loading"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.lineHeight) / 2, 0xffffffff
                );
            } else {
                GuiComponent.drawCenteredString(
                        matrices, textRenderer, Component.translatable("concerto.fail"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.lineHeight) / 2, 0xffffffff
                );
            }
        }
