...
public class ModifiablePressableTextWidget extends PlainTextButton {
    ...
    @Override
    public void renderWidget(PoseStack poseStack, int mouseX, int mouseY, float deltaTicks) {
        Component text = this.isHoveredOrFocused() ? this.hoverText : this.text;
        RenderUtil.renderTextWithShadow(poseStack, this.textRenderer, text, this.getX(), this.getY(), 16777215 | Mth.ceil(this.alpha * 255.0F) << 24);
    }
