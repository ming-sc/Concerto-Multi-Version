            ...
            if (!(options.hideWhenChat && client.screen instanceof ChatScreen)) {
                ...
                if (options.displayLyrics) {
                    ...
                    RenderUtil.renderText(Component.literal(texts[0]), options.lyricsAlignment,
                            pos.x, pos.y, matrices, client.font, (int) config.lyricsColor.getNumber());
                }
                if (options.displaySubLyrics) {
                    ...
                    RenderUtil.renderText(Component.literal(texts[1]), options.subLyricsAlignment,
                            pos.x, pos.y, matrices, client.font, (int) config.subLyricsColor.getNumber());
                }
                ...
                if (options.displayMusicDetails) {
                    ...
                    enableScissor(startX, pos.y, startX + text3Width, pos.y + client.font.lineHeight);
                    RenderUtil.renderText(
                            matrices, client.font, text2, startX + MUSIC_DETAIL_SCROLL.getDx(),
                            pos.y, (int) config.musicDetailsColor.getNumber(), options.textShadow
                    );
                    disableScissor();
                }
                if (options.displayTimeProgress) {
                    ...
                    RenderUtil.renderText(text3, options.timeProgressAlignment,
                            pos.x, pos.y, matrices, client.font, (int) config.timeProgressTextColor.getNumber());
                    ...
                    if (MusicPlayerHandler.INSTANCE.currentMeta != null && MusicPlayerHandler.INSTANCE.currentMeta.getDuration() != null) {
                        ...
                        GuiComponent.fill(matrices, x, pos.y + 3, x + blankWidth - 20, pos.y + 5,
                                (int) config.timeProgressBgColor.getNumber());
                        GuiComponent.fill(matrices, x, pos.y + 3, (int) (x + (blankWidth - 20) * MusicPlayerHandler.INSTANCE.progressPercentage),
