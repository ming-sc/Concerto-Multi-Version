        ...
        if (MusicPlayer.INSTANCE.isPlaying()) {
            ...
            if (!(options.hideWhenChat && client.screen instanceof ChatScreen)) {
                ...
                if (options.displayLyrics) {
                    ...
                    RenderUtil.renderText(new StringTextComponent(texts[0]), options.lyricsAlignment,
                ...
                if (options.displaySubLyrics) {
                    ...
                    RenderUtil.renderText(new StringTextComponent(texts[1]), options.subLyricsAlignment,
                            pos.getX(), pos.getY(), matrices, client.font, (int) config.subLyricsColor.getNumber());
                }

                ITextComponent text3 = new StringTextComponent(texts[3]);
                ...
                if (options.displayMusicDetails) {
                    ...
                    String state = MusicPlayer.INSTANCE.isPlayingTemp ?
                            ConcertoClient.clientState == ConcertoClient.ClientState.MUSIC_AGENT ? " | " + new TranslationTextComponent("concerto.agent").getString() :
                            (ConcertoClient.clientState == ConcertoClient.ClientState.MUSIC_ROOM ? " | " + new TranslationTextComponent("concerto.room").getString() : "")
                            : "";

                    ITextComponent text2 = new StringTextComponent(texts[2] + state);
                ...
                if (options.displayTimeProgress) {
                    ...
                    if (MusicPlayerHandler.INSTANCE.currentMeta != null && MusicPlayerHandler.INSTANCE.currentMeta.getDuration() != null) {
                        ...
                        AbstractGui.fill(matrices, x, pos.getY() + 3, x + blankWidth - 20, pos.getY() + 5,
                                (int) config.timeProgressBgColor.getNumber());
                        AbstractGui.fill(matrices, x, pos.getY() + 3, (int) (x + (blankWidth - 20) * MusicPlayerHandler.INSTANCE.progressPercentage),
                ...
                if (options.displayCoverImg) {
                    ...
                    if (options.coverImgRotate) {
                        ...
                        float angleRad = Util.getMillis() * (float) Math.PI / 180f / 50;
