...
public class ClientMusicNetworkHandler {
        ...
        if (!ConcertoClient.isServerAvailable()) {
            ...
            if (player != null && object != null) {
                ...
                if (packet.to.equals("@a")) {
                    ...
                    player.chat("/msg " + packet.to + " \"" + code + "\"");
                }
            }
            return;
        }
    ...
    public static void accept(PlayerEntity player, UUID uuid, Minecraft client) {
        if (!WAIT_CONFIRMATION.containsKey(uuid)) {
            player.displayClientMessage(new TranslationTextComponent("concerto.confirm.not_found"), false);
        } else {
            MusicDataPacket packet = WAIT_CONFIRMATION.get(uuid);
            MinecraftServer server = client.getSingleplayerServer();
            if (server != null) {
                PlayerEntity from = server.getPlayerList().getPlayerByName(packet.from);
                if (from != null) from.displayClientMessage(new TranslationTextComponent("concerto.confirm.accept_response", player.getName().getString()), false);
            }
            MusicPlayer.INSTANCE.playTempMusic(packet.music);
            WAIT_CONFIRMATION.remove(uuid);
            player.displayClientMessage(new TranslationTextComponent("concerto.confirm.accept"), false);
        }
    }

    public static void rejectAll(PlayerEntity player, Minecraft client) {
        MinecraftServer server = client.getSingleplayerServer();
        WAIT_CONFIRMATION.forEach((uuid, packet) -> {
            if (server != null) {
                PlayerEntity from = server.getPlayerList().getPlayerByName(packet.from);
                if (from != null) from.displayClientMessage(new TranslationTextComponent("concerto.confirm.reject_response", player.getName().getString()), false);
            }
        });
        WAIT_CONFIRMATION.clear();
        player.displayClientMessage(new TranslationTextComponent("concerto.confirm.reject"), false);
    }

    public static void reject(PlayerEntity player, UUID uuid, Minecraft client) {
        if (!WAIT_CONFIRMATION.containsKey(uuid)) {
            player.displayClientMessage(new TranslationTextComponent("concerto.confirm.not_found"), false);
        } else {
            MusicDataPacket packet = WAIT_CONFIRMATION.get(uuid);
    ...
    public static void addToWaitList(Minecraft client, MusicDataPacket packet, PlayerEntity self) {
    ...
    public static void musicDataReceiver(ConcertoPayload payload, NetworkEvent.Context context) {
        try {
            ...
            ClientPlayerEntity self = PlayerUtil.getLocalPlayer();
    ...
    public static void playerJoinHandshake(ConcertoPayload payload, NetworkEvent.Context context) {
            ...
            if (player != null && playerName.equals(player.getName().getString())) {
                ...
                if (args.length > 3 && !Minecraft.getInstance().isLocalServer() && args[3].equals("Invite")) {
                    if (ClientConfig.INSTANCE.options.joinAgentWhenInvited) {
                        player.chat("/musicroom agent join");
                    } else {
                        player.displayClientMessage(CommandUtil.PAGE_SPLIT, false);
                        player.displayClientMessage(new TranslationTextComponent("concerto.agent.invite")
                                .append(new StringTextComponent("  ["))
                                .append(new TranslationTextComponent("concerto.accept").setStyle(
                                        RenderUtil.getRunCommandStyle("/musicroom agent join").withColor(TextFormatting.GREEN)))
    ...
    public static void auditionDataSyncReceiver(ConcertoPayload payload, NetworkEvent.Context context) {
        String str = payload.string;
        String[] args = str.split(";");
        if (args.length != 3) return;
        try {
    ...
    public static void presetRadiosReceiver(ConcertoPayload payload, NetworkEvent.Context context) {
...
//          .peek(playlist -> MusicPlayerHandler.loadInThreadPool(playlist.getList())).toList(), () -> {
    ...
    public static void musicAgentMusicReceiver(ConcertoPayload payload, NetworkEvent.Context context) {
    ...
    public static ClientPlayerEntity getLocalPlayer(PlayerEntity player) {
