...
public class NeteaseCloudLoginScreens extends ConcertoScreen {
    ...
    public static ITextComponent SOURCE_TEXT = new TranslationTextComponent("concerto.source.netease_cloud");
    ...
    protected void init() {
        super.init();
        this.addButton(new ButtonBuilder(new TranslationTextComponent("concerto.screen.login.type.password"),
                button -> Minecraft.getInstance().setScreen(this.passwordLogin())
        ).size(100, 20).pos(this.width / 2 - 50, 40).build());
        this.addButton(new ButtonBuilder(new TranslationTextComponent("concerto.screen.login.type.captcha"),
                button -> Minecraft.getInstance().setScreen(this.captchaLogin())
        ).size(100, 20).pos(this.width / 2 - 50, 70).build());
        this.addButton(new ButtonBuilder(new TranslationTextComponent("concerto.screen.login.type.qrcode"),
                ...
                (username, password) -> {
                    try {
                        ...
                        if (message.getFirst() == 200) {
                            return new TranslationTextComponent("concerto.login.163.success");
                        } else {
                            return new TranslationTextComponent("concerto.login.163.failed", message.getSecond());
                        }
                    } catch (Exception e) {
                        return new TranslationTextComponent("concerto.login.163.error");
                    }
                },
                SOURCE_TEXT,
                this
