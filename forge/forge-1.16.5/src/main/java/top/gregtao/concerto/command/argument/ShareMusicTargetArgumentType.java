...
public class ShareMusicTargetArgumentType implements ArgumentType<String> {
    ...
    public static List<String> getPlayerNameList(ISuggestionProvider provider) {
        List<String> players = new ArrayList<>(provider.getOnlinePlayerNames());
        players.add("@a");
        return players;
    }

    public static String get(CommandContext<CommandSource> context, String key) throws CommandSyntaxException {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        ...
        if (contextSource instanceof ISuggestionProvider provider) {
            return ISuggestionProvider.suggest(getPlayerNameList(provider), builder);
        }
