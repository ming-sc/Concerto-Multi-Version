...
public class ServerMusicNetworkHandler {
    ...
    public static void passAudition(@Nullable ServerPlayerEntity auditor, UUID uuid) {
        if (WAIT_AUDITION.containsKey(uuid)) {
            ...
            if (auditor != null) {
                if (success) {
                    auditor.displayClientMessage(new TranslationTextComponent("concerto.audit.pass", packet.from, packet.music.getMeta().title()), false);
                } else {
                    auditor.displayClientMessage(new TranslationTextComponent("concerto.share.s2c_failed", uuid.toString()), false);
                }
            ...
            auditor.displayClientMessage(new TranslationTextComponent("concerto.audit.uuid_not_found"), false);
        }
    }

    public static void rejectAll(@Nullable ServerPlayerEntity auditor) {
        WAIT_AUDITION.forEach((uuid, packet) -> {
            ServerPlayerEntity player = packet.server.getPlayerList().getPlayerByName(packet.from);
            String title = packet.music.getMeta().title();
            if (player != null) player.displayClientMessage(new TranslationTextComponent("concerto.share.rejected", title), false);
        });
        WAIT_AUDITION.clear();
        if (auditor != null) auditor.displayClientMessage(new TranslationTextComponent("concerto.audit.reject", "ALL", "ALL"), false);
        ConcertoServer.LOGGER.info("Auditor {} rejected all request", auditor == null ? "?" : auditor.getName().getString());
    }

    public static void rejectAudition(@Nullable ServerPlayerEntity auditor, UUID uuid) {
        if (WAIT_AUDITION.containsKey(uuid)) {
            MusicDataPacket packet = WAIT_AUDITION.get(uuid);
            WAIT_AUDITION.remove(uuid);
            PlayerEntity player = packet.server.getPlayerList().getPlayerByName(packet.from);
            String title = packet.music.getMeta().title();
            if (player != null) player.displayClientMessage(new TranslationTextComponent("concerto.share.rejected", title), false);
            if (auditor != null) auditor.displayClientMessage(new TranslationTextComponent(
    ...
    public static boolean sendS2CMusicData(MusicDataPacket packet, boolean audit) {
        ...
        if (packet.to.equals("@a")) {
            ...
            ServerPlayerEntity target = playerManager.getPlayerByName(packet.to);
            ServerPlayerEntity from = playerManager.getPlayerByName(packet.from);
            if (target == null) {
                if (from != null) {
                    from.displayClientMessage(new TranslationTextComponent("concerto.share.s2c_player_not_found", packet.to), false);
                }
                ...
                if (audit && from != null) {
                    from.displayClientMessage(new TranslationTextComponent("concerto.share.audition_passed",
        ...
        try {
            ...
            if (packet != null && packet.music != null && server != null) {
                ...
                if (!playerExist(playerManager, packet.to)) {
                    player.displayClientMessage(new TranslationTextComponent("concerto.share.c2s_player_not_found", packet.to), false);
                    ...
                    if (audit) {
                        ...
                        for (ServerPlayerEntity player1 : playerManager.getPlayers()) {
                    ...
                    player.displayClientMessage(new TranslationTextComponent("concerto.share." + (success ? "success" : "failed")
                ...
                player.displayClientMessage(new TranslationTextComponent("concerto.share.error"), false);
    ...
    public static void musicAgentReceiver(ConcertoPayload payload, NetworkEvent.Context context) {
        ...
        if (!ServerConfig.INSTANCE.options.serverMusicAgent) {
            player.displayClientMessage(new TranslationTextComponent("concerto.agent.not_available"), false);
            return;
        }
        String[] args = payload.string.split(":");
        if (args[0].equals("Join")) {
            ServerMusicAgent.INSTANCE.playerJoin(player);
            player.displayClientMessage(new TranslationTextComponent("concerto.agent.join"), false);
        } else if (args[0].equals("Quit")) {
            ServerMusicAgent.INSTANCE.playerQuit(player);
            player.displayClientMessage(new TranslationTextComponent("concerto.agent.quit"), false);
        } else if (args[0].equals("Query")) {
            List<Music> list = ServerMusicAgent.INSTANCE.getMusicQueue();
            player.displayClientMessage(CommandUtil.PAGE_SPLIT, false);
            list.forEach(music -> player.displayClientMessage(new StringTextComponent(
                    music.getMeta().title() + " - " + music.getMeta().author()), false));
            player.displayClientMessage(CommandUtil.PAGE_SPLIT, false);
        } else if (args.length < 2 || !ServerMusicAgent.INSTANCE.isMember(player)) {
            player.displayClientMessage(new TranslationTextComponent("concerto.agent.error"), false);
    ...
    public static void sendVote2Member(ServerPlayerEntity player) {
        player.displayClientMessage(CommandUtil.PAGE_SPLIT, false);
        player.displayClientMessage(new TranslationTextComponent("concerto.agent.vote")
                .append(new StringTextComponent("  ["))
                .append(new TranslationTextComponent("concerto.accept").setStyle(
                        RenderUtil.getRunCommandStyle("/musicroom agent vote true").withColor(TextFormatting.GREEN)))
                .append(new StringTextComponent("]"))
                .append(new StringTextComponent("  ["))
                .append(new TranslationTextComponent("concerto.reject").setStyle(
                        RenderUtil.getRunCommandStyle("/musicroom agent vote false").withColor(TextFormatting.RED)))
