...
public class MusicCommand {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        LiteralCommandNode<CommandSource> node = dispatcher.register(registerPlayerControllers(
        ...
        if (ClientConfig.INSTANCE.options.registerMusicCommand) {
            dispatcher.register(LiteralArgumentBuilder.<CommandSource>literal("music").redirect(node));
        }
    }
    ...
    private static final List<MusicAdderBuilder.MusicGetter<Music>> GETTERS = List.of(
            context -> {
                ...
                return Pair.of(music, new TranslationTextComponent(Sources.LOCAL_FILE.getKey("add"), music.getRawPath()));
            },
            context -> {
                HttpFileMusic music = new HttpFileMusic(StringArgumentType.getString(context, "path"));
                return Pair.of(music, new TranslationTextComponent(Sources.INTERNET.getKey("add"), music.getRawPath()));
            }
    );

    public static LiteralArgumentBuilder<CommandSource> registerPlayerControllers(
        ...
        return builder.then(
                ClientCommandManager.literal("pause").executes(context -> {
                    if (player.forcePaused) {
                        player.forceResume();
                        CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.resume"));
                    } else {
                        player.forcePause();
                        CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.pause"));
                    }
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("start").executes(context -> {
                    if (!player.started) {
                        player.start();
                        CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.start"));
                    } else {
                        CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.already_started"));
                    }
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("stop").executes(context -> {
                    ...
                    CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.stop"));
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("skip").executes(context -> {
                    MusicPlayer.INSTANCE.stop();
                    CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.skip"));
                    return 0;
                }).then(
                        ClientCommandManager.argument("index", IntegerArgumentType.integer(1)).executes(context -> {
                            ...
                            CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.skip_to", index));
                            return 0;
                        })
                )
        ).then(
                ClientCommandManager.literal("cut").executes(context -> {
                    ...
                    CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.cut"));
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("clear").executes(context -> {
                    ...
                    CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.clear"));
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("restart").executes(context -> {
                    MusicPlayer.resetInstance();
                    CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.success"));
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("mode").then(
                        ClientCommandManager.argument("mode", OrderTypeArgumentType.orderType()).executes((context -> {
                            ...
                            CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.mode", type.getI18nString()));
                            return 0;
                        }))
                )
        ).then(
                ClientCommandManager.literal("reload").executes(context -> {
                    MusicPlayer.INSTANCE.reloadConfig(() ->
                            CommandUtil.commandMessageClient(context, new TranslationTextComponent("concerto.player.reload")));
                ...
                ClientCommandManager.literal("list").then(
                        ClientCommandManager.argument("page", IntegerArgumentType.integer(1)).executes(context -> {
                            ClientPlayerEntity clientPlayer = PlayerUtil.getLocalPlayer();
                            ConcertoRunner.run(() -> {
                                ...
                                for (int i = 10 * (page - 1); i < Math.min(10 * page, list.size()); ++i) {
                                    ...
                                    clientPlayer.displayClientMessage(new StringTextComponent(
                ...
                ClientCommandManager.literal("save").executes(context -> {
                    ...
                    if (MusicPlayerHandler.INSTANCE.currentMusic == null) {
                        clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.unknown"), false);
                        ...
                        ConcertoRunner.run(() -> {
                            try {
                                ...
                                clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.success"), false);
                            } catch (IOException | UnsupportedAudioFileException e) {
                                throw new RuntimeException(e);
                            }
                        });
                    } else {
                        clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.not_cacheable"), false);
                    }
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("like").executes(context -> {
                    ...
                    if (music instanceof Likeable likeable) {
                        CompletableFuture.supplyAsync(likeable::likeIt, ConcertoRunner.RUNNERS_POOL).thenAcceptAsync(success ->
                                clientPlayer.displayClientMessage(success ? new TranslationTextComponent("concerto.like",
                                        music.getMeta().title(), music.getMeta().getSource()) :
                                        new TranslationTextComponent("concerto.fail"), false), ConcertoRunner.RUNNERS_POOL);
                    } else {
                        clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.error.unsupported_operation"), false);
                    }
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("dislike").executes(context -> {
                    ...
                    if (music instanceof Likeable likeable) {
                        CompletableFuture.supplyAsync(likeable::dislikeIt, ConcertoRunner.RUNNERS_POOL).thenAcceptAsync(success ->
                                clientPlayer.displayClientMessage(success ? new TranslationTextComponent("concerto.dislike",
                                        music.getMeta().title(), music.getMeta().getSource()) :
                                        new TranslationTextComponent("concerto.fail"), false), ConcertoRunner.RUNNERS_POOL);
                    } else {
                        clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.error.unsupported_operation"), false);
                ...
                ClientCommandManager.literal("download-current").executes(context -> {
                    ...
                    PlayerUtil.getLocalPlayer().displayClientMessage(new TranslationTextComponent("concerto.success"), false);
                    return 0;
                })
        ).then(
                ...
                ClientCommandManager.literal("export-as-playlist").executes(context -> {
                    ClientPlayerEntity clientPlayer = PlayerUtil.getLocalPlayer();
                    ITextComponent playerName = clientPlayer.getDisplayName();
                    if (PresetPlaylistsConfig.saveToLocalPlaylists(new FixedPlaylist(
                        ...
                        clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.playlist.export.success"), false);
                    } else {
                        clientPlayer.displayClientMessage(new TranslationTextComponent("concerto.playlist.export.fail"), false);
                    }
                    return 0;
                })
        ).then(
    ...
    public static ArgumentBuilder<CommandSource, ?> addMusicCommand() {
                                ...
                                ClientCommandManager.argument("path", StringArgumentType.string()).executes(context -> {
                                    ...
                                    MusicPlayer.INSTANCE.addMusic(
                                            ...
                                            () -> PlayerUtil.getLocalPlayer().displayClientMessage(
                                                    new TranslationTextComponent(Sources.LOCAL_FILE.getKey("add"), path), false)
                                    );
                                    return 0;
                                })
                        )
    ...
    public static ArgumentBuilder<CommandSource, ?> insertMusicCommand() {
