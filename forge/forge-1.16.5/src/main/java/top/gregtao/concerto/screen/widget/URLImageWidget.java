...
public class URLImageWidget implements IRenderable, AutoCloseable {
    ...
    public URLImageWidget(int width, int height, int x, int y, String url) {
        ...
        this.y = y;
        this.url = url;
        this.textureId = new ResourceLocation(ConcertoClient.MOD_ID, "image" + System.currentTimeMillis());
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (this.url == null || this.texture == null) {
            AbstractGui.drawCenteredString(
                matrices,textRenderer, new TranslationTextComponent("concerto.screen.url_image.empty"),
            ...
            if (image != null && this.state == State.READY) {
                ...
                AbstractGui.blit(matrices, this.x, this.y, 0, 0,
                        this.getImageWidth(), this.getImageHeight(), this.getImageWidth(), this.getImageHeight());
                matrices.popPose();
            } else if (this.state == State.LOADING) {
                AbstractGui.drawCenteredString(
                        matrices, textRenderer, new TranslationTextComponent("concerto.screen.loading"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.lineHeight) / 2, 0xffffffff
                );
            } else {
                AbstractGui.drawCenteredString(
                        matrices, textRenderer, new TranslationTextComponent("concerto.fail"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.lineHeight) / 2, 0xffffffff
                );
            }
        }
    ...
    public static void drawBorder(MatrixStack matrices, int x, int y, int width, int height, int color) {
        ...
        AbstractGui.fill(matrices, x, y + 1, x + 1, y + height - 1, color);
        AbstractGui.fill(matrices, x + width - 1, y + 1, x + width, y + height - 1, color);
    }
