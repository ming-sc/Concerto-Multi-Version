...
public class URLImageWidget implements Widget, AutoCloseable {
    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (this.url == null || this.texture == null) {
            GuiComponent.drawCenteredString(
                matrices,textRenderer, new TranslatableComponent("concerto.screen.url_image.empty"),
            ...
            if (image != null && this.state == State.READY) {
                ...
                GuiComponent.blit(matrices, this.x, this.y, 0, 0,
                        this.getImageWidth(), this.getImageHeight(), this.getImageWidth(), this.getImageHeight());
                matrices.popPose();
            } else if (this.state == State.LOADING) {
                GuiComponent.drawCenteredString(
                        matrices, textRenderer, new TranslatableComponent("concerto.screen.loading"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.lineHeight) / 2, 0xffffffff
                );
            } else {
                GuiComponent.drawCenteredString(
                        matrices, textRenderer, new TranslatableComponent("concerto.fail"),
                        this.x + this.width / 2, this.y + (this.height - textRenderer.lineHeight) / 2, 0xffffffff
                );
            }
        }
    ...
    public static void drawBorder(PoseStack matrices, int x, int y, int width, int height, int color) {
