...
public class GeneralPlaylistScreen extends ApplyDraggedFileScreen {
    ...
    public GeneralPlaylistScreen(Screen parent) {
        super(new TranslatableComponent("concerto.screen.general_list"), parent);
    }
    ...
    protected void init() {
        ...
        this.searchBox = new EditBox(this.font, this.width / 2 - 185, 18, 300, 18,
                this.searchBox, new TranslatableComponent("concerto.screen.search"));
        this.addWidget(this.searchBox);
        this.addRenderableWidget(this.searchBox);

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.search"), button ->
                this.toggleSearch()).pos(this.width / 2 + 125, 17).size(50, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.next"), button -> {
        ...
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.delete"), button -> {
            ConcertoListWidget<Music>.Entry entry = this.widget.getSelected();
            if (entry != null) {
        ...
        this.addRenderableWidget(CycleButton.<OrderType>builder((type) -> new TextComponent(type.getI18nString())).withValues(OrderType.values())
                .withInitialValue(MusicPlayerHandler.INSTANCE.getOrderType()).create(
                        this.width / 2 - 35, this.height - 30, 60, 20, new TranslatableComponent("concerto.screen.order"),
                        (widget, orderType) -> MusicPlayerHandler.INSTANCE.setOrderType(orderType)));

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.pause"), button -> {
        ...
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.info"), button -> {
            ConcertoListWidget<Music>.Entry entry = this.widget.getSelected();
            if (entry != null) {
        ...
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.clear"), button -> {
