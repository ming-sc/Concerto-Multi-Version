...
public class ConcertoIndexScreen extends ConcertoScreen {
    public ConcertoIndexScreen(Screen parent) {
        super(new TranslatableComponent("concerto.screen.index.title"), parent);
    }

    @Override
    protected void init() {
        super.init();
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.general_list"),
                button -> Minecraft.getInstance().setScreen(new GeneralPlaylistScreen(this))
        ).pos(this.width / 2 - 120, 20).size(115, 20).build());

        Button widget = new ButtonBuilder(new TranslatableComponent("concerto.screen.audition"),
        ...
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.confirmation"),
                button -> Minecraft.getInstance().setScreen(new MusicConfirmationScreen(this))
        ).pos(this.width / 2 + 5, 50).size(115, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.index.163"),
                button -> Minecraft.getInstance().setScreen(new NeteaseCloudIndexScreen(this))
        ).pos(this.width / 2 - 120, 50).size(115, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.index.qq"),
                button -> Minecraft.getInstance().setScreen(new QQMusicIndexScreen(this))
        ).pos(this.width / 2 - 120, 80).size(115, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.index.kugou"),
                button -> Minecraft.getInstance().setScreen(new KuGouMusicIndexScreen(this))
        ).pos(this.width / 2 - 120, 110).size(115, 20).build());

        Button widget1 = new ButtonBuilder(new TranslatableComponent("concerto.screen.preset_radios"),
        ...
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.add"),
                button -> Minecraft.getInstance().setScreen(new AddMusicScreen(this)))
                .pos(this.width / 2 - 120, 140).size(115, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.local_playlists"),
                button -> Minecraft.getInstance().setScreen(new PlaylistListScreen(
                        new TranslatableComponent("concerto.screen.local_playlists"), this, PresetPlaylistsConfig.LOCAL_PLAYLISTS.getRadios()))
        ).pos(this.width / 2 + 5, 110).size(115, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.report_bugs"),
                button -> Util.getPlatform().openUri("https://github.com/GregTaoo/Concerto/issues")
        ).pos(this.width / 2 + 5, 140).size(115, 20).build());

        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.options"),
        ...
        if (this.minecraft != null) {
            switch (ConcertoClient.clientState) {
                case MUSIC_ROOM -> {
                    ...
                    Component text = new TranslatableComponent("concerto.screen.in_music_room", uuid);
                    int width = this.font.width(text);
                    this.addRenderableWidget(new PressableTextWidget(
                ...
                case MUSIC_AGENT -> {
                    Component text = new TranslatableComponent("concerto.screen.in_music_agent");
                    int width = this.font.width(text);
    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (ConcertoClient.clientState != ConcertoClient.ClientState.LOCAL) {
            RenderUtil.renderCenteredString(
                    matrices,
                    this.font,
                    new TranslatableComponent("concerto.screen.in_which_room"),
