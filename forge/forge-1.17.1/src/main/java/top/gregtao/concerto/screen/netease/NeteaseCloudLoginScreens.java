...
public class NeteaseCloudLoginScreens extends ConcertoScreen {
    ...
    public static Component SOURCE_TEXT = new TranslatableComponent("concerto.source.netease_cloud");

    public NeteaseCloudLoginScreens(Screen parent) {
        super(new TextComponent(new TranslatableComponent("concerto.screen.login").getString() + SOURCE_TEXT.getString()), parent);
    }

    @Override
    protected void init() {
        super.init();
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.login.type.password"),
                button -> Minecraft.getInstance().setScreen(this.passwordLogin())
        ).size(100, 20).pos(this.width / 2 - 50, 40).build());
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.login.type.captcha"),
                button -> Minecraft.getInstance().setScreen(this.captchaLogin())
        ).size(100, 20).pos(this.width / 2 - 50, 70).build());
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.login.type.qrcode"),
                button -> Minecraft.getInstance().setScreen(this.qrCodeLogin())
        ).size(100, 20).pos(this.width / 2 - 50, 100).build());
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.login.type.cookie"),
                ...
                (username, password) -> {
                    try {
                        ...
                        if (message.getFirst() == 200) {
                            return new TranslatableComponent("concerto.login.163.success");
                        } else {
                            return new TranslatableComponent("concerto.login.163.failed", message.getSecond());
                        }
                    } catch (Exception e) {
                        return new TranslatableComponent("concerto.login.163.error");
                    }
                },
                SOURCE_TEXT,
                this
