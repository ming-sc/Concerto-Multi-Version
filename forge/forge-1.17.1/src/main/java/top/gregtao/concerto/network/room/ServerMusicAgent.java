...
public class ServerMusicAgent {
    ...
    public void receiveVoteRequest(ServerPlayer player) {
        if (this.isVoting) {
            player.displayClientMessage(new TranslatableComponent("concerto.agent.vote.voting"), false);
            return;
        }
        if (!this.isPlaying.get()) {
            player.displayClientMessage(new TranslatableComponent("concerto.agent.not_playing"), false);
            return;
        }
    ...
    public void receiveVote(ServerPlayer player, boolean vote) {
        if (!this.isVoting) {
            player.displayClientMessage(new TranslatableComponent("concerto.agent.vote.ended"), false);
            return;
        }

        synchronized (this) {
            if (this.yesVoters.contains(player) || this.noVoters.contains(player)) {
                player.displayClientMessage(new TranslatableComponent("concerto.agent.vote.duplicate"), false);
                return;
            }
            this.voteLock.lock();
            ...
            this.voteLock.unlock();
            player.displayClientMessage(new TranslatableComponent("concerto.agent.vote_for", vote ?
                    new TranslatableComponent("concerto.accept") : new TranslatableComponent("concerto.reject")), false);
    ...
    private synchronized void endVoting() {
        ...
        Component text = new TranslatableComponent(success ? "concerto.agent.vote.success" : "concerto.agent.vote.failed",
    ...
    public synchronized void playNextMusic() {
        try {
            ...
            if (this.currentMusic == null) {
                ...
                if (ServerConfig.INSTANCE.options.musicAgentUseShared && this.currentMusic instanceof DynamicPath dynamicPath) {
                    ...
                    if (path != null) {
                        ...
                        this.broadcast(new TranslatableComponent("concerto.agent.play.failed",
            ...
            this.broadcast(new TranslatableComponent("concerto.agent.error"));
            this.reset();
        }
    }
    ...
    public synchronized void addMusic(ServerPlayer player, Music music) {
        ...
        if (wait > 0) {
            player.displayClientMessage(new TranslatableComponent("concerto.agent.add.too_quick", wait), false);
            return;
        }
        ConcertoRunner.run(() -> {
            ...
            this.broadcast(new TranslatableComponent("concerto.agent.add",
                    player == null ? new TranslatableComponent("concerto.unknown") : player.getName().getString(),
