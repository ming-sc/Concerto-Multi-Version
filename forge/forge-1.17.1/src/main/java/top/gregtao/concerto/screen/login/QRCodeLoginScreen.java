...
public class QRCodeLoginScreen extends ConcertoScreen {
    ...
    protected void init() {
        super.init();
        this.addRenderableWidget(new ButtonBuilder(new TranslatableComponent("concerto.screen.login.qrcode.refresh"), button -> {
            this.timer = 0;
    ...
    public void tick() {
        ...
        if (this.timer == 0) {
            switch (this.status) {
                ...
                case FAILED -> this.loadQRCode(new TranslatableComponent("concerto.screen.login.qrcode.failed"));
                case EXPIRED -> this.loadQRCode(new TranslatableComponent("concerto.screen.login.qrcode.expired"));
                case SUCCESS -> {
                    LocalPlayer player = PlayerUtil.getLocalPlayer();
                    if (player != null) {
                        player.displayClientMessage(new TranslatableComponent("concerto.screen.login.qrcode.success"), false);
                    }
