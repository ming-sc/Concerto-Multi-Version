    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(
                Commands.literal("concerto-server").then(
                        Commands.literal("audit").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS)).then(
                        ...
                        Commands.literal("reload").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))
                                .executes(context -> {
                                    ConcertoServer.reload();
                                    return 0;
                                })
                ).then(
                        Commands.literal("reload-cookie").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))
                                .executes(context -> {
                        ...
                        Commands.literal("clean-cache").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))
                                .executes(context -> {
                        ...
                        Commands.literal("fetch-radios")
                                .requires(Commands.hasPermission(Commands.LEVEL_ALL)).executes(context -> {
                                    ServerPlayer player = context.getSource().getPlayer();
                                    if (player != null) ServerMusicNetworkHandler.sendS2CPresetRadiosPacket(player);
                                    return 0;
                                })
                ).then(
                        Commands.literal("agent").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS)).then(
