...
public class ClientMusicNetworkHandler {
    ...
    public static void sendC2SMusicData(MusicDataPacket packet) {
        ...
        ClientPacketDistributor.sendToServer(buf);
    }

    public static void accept(Player player, UUID uuid, Minecraft client) {
        if (!WAIT_CONFIRMATION.containsKey(uuid)) {
            CommandUtil.displayClientMessage(player, Component.translatable("concerto.confirm.not_found"), false);
        } else {
            MusicDataPacket packet = WAIT_CONFIRMATION.get(uuid);
            MinecraftServer server = client.getSingleplayerServer();
            if (server != null) {
                Player from = server.getPlayerList().getPlayerByName(packet.from);
                if (from != null) CommandUtil.displayClientMessage(from, Component.translatable("concerto.confirm.accept_response", player.getName().getString()), false);
            }
            MusicPlayer.INSTANCE.playTempMusic(packet.music);
            WAIT_CONFIRMATION.remove(uuid);
            CommandUtil.displayClientMessage(player, Component.translatable("concerto.confirm.accept"), false);
        }
    }
    ...
    public static void rejectAll(Player player, Minecraft client) {
        ...
        WAIT_CONFIRMATION.forEach((uuid, packet) -> {
            if (server != null) {
                Player from = server.getPlayerList().getPlayerByName(packet.from);
                if (from != null) CommandUtil.displayClientMessage(from, Component.translatable("concerto.confirm.reject_response", player.getName().getString()), false);
            }
        });
        WAIT_CONFIRMATION.clear();
        CommandUtil.displayClientMessage(player, Component.translatable("concerto.confirm.reject"), false);
    }
    ...
    public static void addToWaitList(Minecraft client, MusicDataPacket packet, Player self) {
        ...
        ConcertoRunner.run(() -> {
            if (ClientConfig.INSTANCE.options.confirmAfterReceived) {
                CommandUtil.displayClientMessage(self, CommandUtil.PAGE_SPLIT, false);
                CommandUtil.displayClientMessage(self, ShareMusicCommand.chatMessageBuilder(uuid, packet.from, packet.music.getMeta().title()), false);
                CommandUtil.displayClientMessage(self, CommandUtil.PAGE_SPLIT, false);
            } else {
            ...
            if (player != null && playerName.equals(player.getName().getString())) {
                ...
                if (args.length > 3 && !Minecraft.getInstance().isSingleplayer() && args[3].equals("Invite")) {
                    if (ClientConfig.INSTANCE.options.joinAgentWhenInvited) {
                        ...
                        CommandUtil.displayClientMessage(player, CommandUtil.PAGE_SPLIT, false);
                        CommandUtil.displayClientMessage(player, Component.translatable("concerto.agent.invite")
...
//          .peek(playlist -> MusicPlayerHandler.loadInThreadPool(playlist.getList())).toList(), () -> {
    ...
    public static void musicAgentJoin() {
        ClientPacketDistributor.sendToServer(new ConcertoPayload(ConcertoPayload.Channel.MUSIC_AGENT, "Join"));
        ConcertoClient.clientState = ConcertoClient.ClientState.MUSIC_AGENT;
    }

    public static void musicAgentQuit() {
       ClientPacketDistributor.sendToServer(new ConcertoPayload(ConcertoPayload.Channel.MUSIC_AGENT, "Quit"));
        ConcertoClient.clientState = ConcertoClient.ClientState.LOCAL;
    }

    public static void musicAgentNewVote() {
       ClientPacketDistributor.sendToServer(new ConcertoPayload(ConcertoPayload.Channel.MUSIC_AGENT, "Vote:New"));
    }

    public static void musicAgentQuery() {
       ClientPacketDistributor.sendToServer(new ConcertoPayload(ConcertoPayload.Channel.MUSIC_AGENT, "Query"));
    }

    public static void musicAgentVote(boolean vote) {
       ClientPacketDistributor.sendToServer(new ConcertoPayload(ConcertoPayload.Channel.MUSIC_AGENT, "Vote:" + (vote ? "1" : "0")));
    }
    ...
    public static boolean musicAgentAddMusic(Music music) {
       ...
       ClientPacketDistributor.sendToServer(new ConcertoPayload(ConcertoPayload.Channel.MUSIC_AGENT,
