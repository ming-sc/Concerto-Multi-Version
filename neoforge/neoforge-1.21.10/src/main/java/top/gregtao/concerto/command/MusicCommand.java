        ...
        return builder.then(
                ...
                ClientCommandManager.literal("list").then(
                        ClientCommandManager.argument("page", IntegerArgumentType.integer(1)).executes(context -> {
                            ...
                            ConcertoRunner.run(() -> {
                                ...
                                CommandUtil.displayClientMessage(clientPlayer, CommandUtil.PAGE_SPLIT, false);

                                for (int i = 10 * (page - 1); i < Math.min(10 * page, list.size()); ++i) {
                                    MusicMetaData meta = list.get(i).getMeta();
                                    CommandUtil.displayClientMessage(clientPlayer, Component.literal(
                ...
                ClientCommandManager.literal("save").executes(context -> {
                    ...
                    if (MusicPlayerHandler.INSTANCE.currentMusic == null) {
                        CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.unknown"), false);
                        ...
                        ConcertoRunner.run(() -> {
                            try {
                                ...
                                CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.success"), false);
                            } catch (IOException | UnsupportedAudioFileException e) {
                                throw new RuntimeException(e);
                            }
                        });
                    } else {
                        CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.not_cacheable"), false);
                    }
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("like").executes(context -> {
                    ...
                    if (music instanceof Likeable likeable) {
                        CompletableFuture.supplyAsync(likeable::likeIt, ConcertoRunner.RUNNERS_POOL).thenAcceptAsync(success ->
                                CommandUtil.displayClientMessage(clientPlayer, success ? Component.translatable("concerto.like",
                                        music.getMeta().title(), music.getMeta().getSource()) :
                                        Component.translatable("concerto.fail"), false), ConcertoRunner.RUNNERS_POOL);
                    } else {
                        CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.error.unsupported_operation"), false);
                    }
                    return 0;
                })
        ).then(
                ClientCommandManager.literal("dislike").executes(context -> {
                    ...
                    if (music instanceof Likeable likeable) {
                        CompletableFuture.supplyAsync(likeable::dislikeIt, ConcertoRunner.RUNNERS_POOL).thenAcceptAsync(success ->
                                CommandUtil.displayClientMessage(clientPlayer, success ? Component.translatable("concerto.dislike",
                                        music.getMeta().title(), music.getMeta().getSource()) :
                                        Component.translatable("concerto.fail"), false), ConcertoRunner.RUNNERS_POOL);
                    } else {
                        CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.error.unsupported_operation"), false);
                ...
                ClientCommandManager.literal("download-current").executes(context -> {
                    ...
                    CommandUtil.displayClientMessage(PlayerUtil.getLocalPlayer(), Component.translatable("concerto.success"), false);
                    return 0;
                })
        ).then(
                ...
                ClientCommandManager.literal("export-as-playlist").executes(context -> {
                    ...
                    if (PresetPlaylistsConfig.saveToLocalPlaylists(new FixedPlaylist(
                        ...
                        CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.playlist.export.success"), false);
                    } else {
                        CommandUtil.displayClientMessage(clientPlayer, Component.translatable("concerto.playlist.export.fail"), false);
                    }
                    return 0;
                })
        ).then(
                        ...
                        ClientCommandManager.literal("folder").then(
                                ClientCommandManager.argument("path", StringArgumentType.string()).executes(context -> {
                                    ...
                                    MusicPlayer.INSTANCE.addMusic(
                                            ...
                                            () -> CommandUtil.displayClientMessage(
                                                    PlayerUtil.getLocalPlayer(),
